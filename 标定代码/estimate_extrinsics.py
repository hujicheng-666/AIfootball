...
PRESET_TASKS = {
    "left": {
        ...
        "view_type": "view_right_side",
        "output_prefix": "left",
        "description": "Left camera reference image (站在球门看向场地右侧)",
        "intrinsics_profile": "left",
    },
    "right": {
        "image_path": BASE_DIR / "IMG_20260314_222612.jpg",
        "view_type": "view_left_side",
        "output_prefix": "right",
        "description": "Right camera reference image (站在球门看向场地左侧)",
...
POINT_LABELS = {
    "left_post_bottom": "left post base",
    "right_post_bottom": "right post base",
    "left_post_top": "left post top",
    "right_post_top": "right post top",
    "penalty_spot": "penalty spot",
    "left_goal_area_corner": "left goal-area corner",
    "right_goal_area_corner": "right goal-area corner",
}

# view_type 命名规则（球门视角坐标系）：
#   "view_left_side"  = 从球门看向场地左侧 (右相机) — 点球点 + 右侧小禁区角点可见
#   "view_right_side" = 从球门看向场地右侧 (左相机) — 点球点 + 左侧小禁区角点可见
POINT_ORDERS = {
    "view_left_side": [
        "left_post_bottom",
        "right_post_bottom",
...
def estimate_extrinsics(object_points, image_points, camera_matrix, dist_coeffs):
    methods = [
    ...
    if not ok:
        raise RuntimeError("solvePnPRansac failed and solvePnP fallback also failed")

    rvec, tvec = refine_pose(object_points, image_points, camera_matrix, dist_coeffs, rvec, tvec)
    return {
        "solver_name": "solvePnP(ITERATIVE fallback)",
        "rvec": rvec,
        "tvec": tvec,
...
def resolve_task(task_name, args):
    ...
    view_type = input("Enter view type (view_left_side / view_right_side): ").strip()
    if view_type not in POINT_ORDERS:
        raise ValueError("View type must be view_left_side or view_right_side")
...
def choose_tasks(args):
    ...
    print("Task options:")
    print("  left  - process the left preset image with old intrinsics")
    print("  right - process the right preset image with new intrinsics")
...
def process_task(meta):
    ...
    result = estimate_extrinsics(object_points, image_points, working_camera_matrix, intrinsics_bundle.dist_coeffs)
    rvec = result["rvec"]
    tvec = result["tvec"]
